...
const CourseArticulationMatrix = () => {
  ...
  // Function to calculate correlation value based on percentage
  const calculateCorrelation = (percentage) => {
    if (percentage <= 5) {
      return 0;
    } else if (percentage <= 40) {
      return 1;
    } else if (percentage < 60.1) {
      return 2;
    } else {
      return 3;
    }
      ...
      <div className="container-fluid vh-100 vw-100 custom-table overflow-auto">
        <div className="container-fluid bg-dark text-white cusTable">
          <div className="row bg-dark text-white no-wrap fs-4 d-flex justify-content-center font-weight-bold">
            ...
            {/* Additional column for correlation */}
            <div className="col">
              Correlation
            </div>
          </div>
          ...
          {Array.isArray(data) && data.map((item, index) => (
            <div className="row" key={index}>
              ...
              {/* Calculate and display correlation values for each PO column */}
              {Array.isArray(item.poPercentage) && item.poPercentage.map((poPercentage, poIndex) => (
                <div className="col" key={poIndex}>
                  {calculateCorrelation(poPercentage)} {/* Display calculated correlation */}
                </div>
              ))}
              
              {/* Calculate and display correlation values for each PSO column */}
              {Array.isArray(item.psoPercentage) && item.psoPercentage.map((psoPercentage, psoIndex) => (
                <div className="col" key={psoIndex}>
                  {calculateCorrelation(psoPercentage)} {/* Display calculated correlation */}
                </div>
              ))}
